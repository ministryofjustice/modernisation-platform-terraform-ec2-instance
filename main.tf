...
resource "aws_iam_role_policy_attachment" "this" {
  count      = length(distinct(concat(["arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"], var.instance_profile_policies)))
  role       = aws_iam_role.this.name
  policy_arn = element(distinct(concat(["arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"], var.instance_profile_policies)), count.index)
}
